...
<body>
 ...
 %>
	...
	<div class="row">
			...
			<div class="card mx-auto my-2">
				<div class="card-body">
					...
					<form action="AddExpenseServlet" method="POST">
						...
						<small id="investmentHelp" class="form-text text-muted mb-3">Check the box for recurring expenses</small>
		...
		<div class="col">
			<c:if test="${ user.getExpenses() != null }">
				<table class="table table-dark table-hover my-2 rounded-lg">
					...
					<c:forEach var="investment" items="${user.getInvestments()}">
						...
						<tr name="investments">
							...
							<c:url value="RemoveExpenseServlet" var="myURL"> 
								<c:param name = "removeAmount" value = "${investment.getAmount()}"/>
								<c:param name = "removeRecurring" value = "${investment.getRecurring()}"/>
							</c:url>
